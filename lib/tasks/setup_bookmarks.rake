# 书签设置任务 - Bookmarks setup task
namespace :bookmarks do
  desc "Setup bookmarks table and seed data"
  task setup: :environment do
    puts "Setting up bookmarks table..."
    
    # 检查表是否已存在 - Check if table already exists
    if ActiveRecord::Base.connection.table_exists?('bookmarks')
      puts "Bookmarks table already exists, skipping creation."
    else
      puts "Creating bookmarks table..."
      
      # 创建表 - Create table
      ActiveRecord::Base.connection.create_table :bookmarks do |t|
      ...
      # 添加索引 - Add indexes
      ActiveRecord::Base.connection.add_index :bookmarks, :category
      ActiveRecord::Base.connection.add_index :bookmarks, :featured
      ActiveRecord::Base.connection.add_index :bookmarks, :published
      ActiveRecord::Base.connection.add_index :bookmarks, :published_at
      
      puts "Bookmarks table created successfully!"
    end
    
    # 种子数据 - Seed data
    puts "Seeding bookmarks data..."
    ...
    bookmarks_data.each do |bookmark_attrs|
      bookmark = Bookmark.find_or_create_by(url: bookmark_attrs[:url]) do |b|
        ...
        b.tags = bookmark_attrs[:tags] # Will be converted to JSON by the model
    ...
    puts "Bookmarks setup completed!"
    puts "Created #{Bookmark.count} bookmarks"
    puts "Featured bookmarks: #{Bookmark.where(featured: true).count}"
    puts "Categories: #{Bookmark.group(:category).count}"
  end
  
  desc "Reset bookmarks data"
  task reset: :environment do
    puts "Resetting bookmarks data..."
    ...
    Rake::Task['bookmarks:setup'].invoke
  end
end
